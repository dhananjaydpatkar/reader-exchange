...
async function testLocalityFlow() {
    try {
        ...
        const brahmand = locRes.data.find((l) => l.name === 'Brahmand');
        const lodha = locRes.data.find((l) => l.name === 'Lodha Amara');
        if (!brahmand || !lodha) {
        ...
        try {
            await axios.post(`${BASE_URL}/exchange`, { bookId, type: 'exchange' }, { headers: { Authorization: `Bearer ${tokenA}` } });
        ...
        try {
            await axios.post(`${BASE_URL}/exchange`, { bookId: bookAId, type: 'exchange' }, { headers: { Authorization: `Bearer ${tokenB}` } });
            console.error('❌ FAILURE: Request should have been rejected!');
        }
        catch (error) {
            if (error.response && error.response.status === 400 && error.response.data.message.includes('not yet live')) {
...
testLocalityFlow();
//# sourceMappingURL=test_locality_flow.js.map