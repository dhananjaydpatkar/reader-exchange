...
export const listBooks = async (req, res) => {
    try {
        ...
        if (search) {
            queryBuilder.andWhere('(book.title ILIKE :search OR book.author ILIKE :search OR book.genre ILIKE :search OR owner.name ILIKE :search)', { search: `%${search}%` });
        }
...
export const relistBook = async (req, res) => {
    const { id } = req.params;
    const userId = req.user.id;
